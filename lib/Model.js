...
function Model(opts) {
	...
	var createInstance = function (data, inst_opts, cb) {
		...
		var filteredOneAssociations;
		var filteredManyAssociations;
		if (!inst_opts.autoFetchNames) {
			filteredOneAssociations = one_associations;
			filteredManyAssociations = many_associations;
		} else {
			filteredOneAssociations = _.filter(one_associations, function(association) {
				return inst_opts.autoFetchNames.indexOf(association.name) >= 0;
			});
			filteredManyAssociations = _.filter(many_associations, function(association) {
				return inst_opts.autoFetchNames.indexOf(association.name) >= 0;
			});

		}

		OneAssociation.autoFetch(instance, filteredOneAssociations, assoc_opts, function () {
			ManyAssociation.autoFetch(instance, filteredManyAssociations, assoc_opts, function () {
	...
	//I think GET should be refactored to just call FIND!
	model.get = function () {
		var conditions = {};
		var options    = {};
		...
		opts.driver.find(model_fields, opts.table, conditions, { limit: 1 }, function (err, data) {
			...
			Singleton.get(uid, {
				...
				return createInstance(data[0], {
					...
					autoFetchNames : options.autoFetchNames,
